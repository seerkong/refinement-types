...
import link.symtable.refinementtypes.errors.AllFailedError;
...
/**
 * or关系，如果校验失败时，返回所有校验失败的元素
 */
public class OrValidatorMaker {
    public static AnyValidator make(AnyValidator... validators) {
        ...
        return input -> {
            ...
            // 如果有一个成功则认为校验成功
            if (errors.size() < validators.length) {
                return new ArrayOutput(input, null);
            } else {
                return new ArrayOutput(null, new AllFailedError(errors));
            }
        };
    }
}
